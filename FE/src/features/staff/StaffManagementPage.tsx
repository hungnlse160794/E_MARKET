...
export default function StaffManagementPage() {
  ...
  const { data: staffData, isLoading: isListLoading } = useStaff(staffFilter);
  ...
  const handleAssignSubmit = useCallback((managedBranches: string[]) => {
    if (assigningStaff) {
  ...
  }, [assigningStaff, updateManagedBranchesMutation]);

  const handleAssignClose = useCallback(() => setIsAssignModalOpen(false), []);
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        {isListLoading ? (
            ...
            <StaggerContainer staggerDelay={0.1} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                {stats.map((stat) => (
                <MotionWrapper key={stat.label} variant="staggerItem" className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-5">
                        ...
                        <div className="flex flex-col items-center">
                          {staff.role === 'BRANCH_MANAGER' ? (
                            <div className="flex flex-wrap justify-center gap-1 max-w-[150px]">
                              {Array.isArray(staff.managedBranches) && staff.managedBranches.length > 0 ? (
                                staff.managedBranches.map((mb, i) => (
